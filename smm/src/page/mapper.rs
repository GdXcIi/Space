...
impl<A: Arch, F> PageMapper<A, F> {
    ...
    pub fn table(&self) -> PageTable<A> {
        // SAFETY: The only way to initialize a PageMapper is via new(), and I assume it upholds
    ...
    pub fn translate(&self, virt: VirtualAddress) -> Option<(PhysicalAddress, PageFlags<A>)> {
        let entry = self.visit(virt, |p1, i| unsafe { p1.entry(1) })??;
    ...
    pub unsafe fn remap_with_full(
        ...
        unsafe {
            self.visit(virt, |p1, i| {
                ...
                // TODO: high level PageEntry::new interface
...
impl<A: Arch, F: FrameAllocator> PageMapper<A, F> {
    pub unsafe fn create(table_kind: TableKind, mut allocator: F) -> Option<Self> {
        unsafe {
            ...
            match (table_kind, A::KERNEL_SEPARATE_TABLE) {
                ...
                (_, true) => {
                    // there is a separate page table for the kernel.
                    // no need to copy the kernel mappings to the user page table
                }
            }
    ...
    pub unsafe fn map_phys(
        ...
        unsafe {
            // TODO: verifiy virt and phys are aligned
            ...
            loop {
                let i = table.index_of(virt)?;
                if table.level() == 0 {
                    // TODO: check for overwritting entry
    ...
    unsafe fn unmap_phys_inner<A: Arch>(
        ...
        if unmap_parents {
            ...
            let is_still_populated = (0..A::PAGE_ENTRIES)
                .map(|j| subtable.entry(j).expect("must be with bounds"))
                .any(|e| e.present());
