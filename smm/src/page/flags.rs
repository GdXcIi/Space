...
impl<A: Arch> PageFlags<A> {
    ...
    pub fn new_table() -> Self {
        unsafe {
            Self::from_data(
                // Flags set to present, kernel space, read only, no execute by default
                A::ENTRY_FLAG_DEFAULT | A::ENTRY_FLAG_NO_EXEC | A::ENTRY_FLAG_NO_GLOBAL,
            )
        }
    }
    ...
    pub fn uncacheable(self, value: bool) -> Self {
        self.custom_flag(A::ENTRY_FLAG_UNCACHEABLE)
    }
    ...
    pub fn write(self, value: bool) -> Self {
        // Architecture may use read only or readwrite, or both, support either
        if value {
